...
if(!empty($_POST)  && empty($error)) {
	...
	if(empty($post_data['error'])) {
		...
		try{
			...
			$result = $FlashFotoAPI->mugshot($image_id);
		
			sleep(15);
			header('Location: fun.php?i='.$image_id);
			exit;
...
<html>
	<head>
		<title><?php echo ucwords($method); ?> stolen from Examples - FlashFoto PHP API SDK</title>
	...
	<body>
		...
		<h2>warning, i'm slow... please be patient</h2>
